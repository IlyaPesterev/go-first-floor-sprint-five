...
type Training struct {
	...
	Action       int           // количество повторов(шаги при беге и ходьбе, гребки при плавании)
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	ccalRunn := ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
	return ccalRunn
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	seedMsec := w.meanSpeed() * KmHInMsec
	seedPow := math.Pow(seedMsec, 2)
	ccalWalk := ((CaloriesWeightMultiplier*w.Weight + seedPow/(w.Height/CmInM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return ccalWalk
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	seedSwimm := float64(s.LengthPool*s.CountPool) / MInKm / s.Duration.Hours()
	return seedSwimm
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	ccalSwimm := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return ccalSwimm
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	infoMessage.Distance = float64(s.LengthPool*s.CountPool) / MInKm
